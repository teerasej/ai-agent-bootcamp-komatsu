...
// SubmitPv tool — stub implementation to be replaced with Cosmos DB insertion
    ...
    string pvJson)
{
    ...
    //   1. Parse pvJson using JsonNode.Parse()
    //   2. Extract the "pv" object (or use the root if no wrapper)
    //   3. Add a unique "id" field using Guid.NewGuid().ToString()
    //   4. Create a CosmosClient using the connectionString
    //   5. Get the container and call CreateItemStreamAsync() with the document bytes
    //   6. Return a success message with the new document id
    Console.WriteLine("\n--- PV Submission Received (stub) ---");
