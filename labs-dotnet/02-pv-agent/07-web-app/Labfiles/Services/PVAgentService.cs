...
/// <summary>
/// Wraps the PV Agent — one instance per Blazor circuit (scoped).
/// </summary>
public class PVAgentService
{
    ...
    public PVAgentService(IConfiguration configuration)
    {
        ...
        // [TODO] Create the AIAgent using OpenAIClient, similar to the console app in exercise 06.
        //   1. Create an OpenAIClient with the apiKey and endpoint
        //   2. Call .GetChatClient(deploymentName)
        //   3. Call .AsAIAgent() with:
        //      - instructions: PVAgentInstructions (defined at the bottom of this file)
        //      - name: "PVAgent"
        //      - tools: array containing AIFunctionFactory.Create(GetProjectBudget) and AIFunctionFactory.Create(SubmitPv)
        _agent = null!; // ← Replace this line

        // [TODO] Create an AgentSession from the agent
        //   Hint: _agent.CreateSessionAsync().GetAwaiter().GetResult()
        _session = null!; // ← Replace this line
    }

    /// <summary>
    /// Sends a user message and returns the full agent response.
    /// </summary>
    public async Task<string> SendMessageAsync(string userMessage)
    {
        // [TODO] Implement this method:
        //   1. Create a StringBuilder
        //   2. Use "await foreach" to iterate over _agent.RunStreamingAsync(userMessage, _session)
        //   3. Append each update.Text to the StringBuilder
        ...
        string pvJson)
    {
        ...
        var databaseName = _configuration["CosmosDB:DatabaseName"] ?? "pv-agent-db";
        var containerName = _configuration["CosmosDB:ContainerName"] ?? "payment-vouchers";
    ...
    private const string PVAgentInstructions = """
